...
yum install sudo -y 
sudo yum -y update
ln -snf /usr/share/zoneinfo/Asia/Shanghai /etc/localtime && sudo yum install -y git && npm config set registry https://registry.npmmirror.com
...
curl -fsSL https://raw.githubusercontents.com/tj/n/master/bin/n | sudo -E bash -s lts
